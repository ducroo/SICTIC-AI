...
> Named `AGENT_RULES.md` to distinguish these coding-agent rules from the
> repository-level working instructions in `AGENTS.md`.

**Paste this to your coding agent before it writes anything, every session.**

You are contributing to SICTIC-AI, a shared codebase with multiple contributors.
Most of what you are asked to build already exists in some form. Your job is to
find it and compose it, not to write it again.
...
1. **Work from current `main`.** `git fetch origin && git merge origin/main`.
   ...
   routine you are about to write may have been added recently.
...
3. **Look for what already exists.** `ls skills/` lists the repository's
...
4. **Verify every name against the source.** If a document, a table, or a
   ...
   signature. A reuse list is only as good as the tree it was checked against.
...
* **Do not print.** Use `lib.infrastructure.logging.get_logger`. `print()` is
  reserved for final output to the user.
* **Do not weaken a check to get green.** Not a test, not a linter, not a gate.
...
| one free-form model call | `lib.infrastructure.ai_text_generation.generate_markdown` |
...
| resolve a LinkedIn profile | `lib.people.linkedin.service.LinkedInResolver` |
