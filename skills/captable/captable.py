"""One managed Markdown capitalization report over consolidated build data."""
    ...
    *,
    ...
    if pre_money is None:
        if caps:
            ...
            multiplier = settings["fallback_pre_money_invested_multiplier"]
            minimum = settings["fallback_pre_money_minimum"]
            pre_money = max(multiplier * invested, minimum)
            assumptions.append(
                f"No valuation cap available; hypothetical pre-money set "
                f"to {multiplier:g}x cumulative invested capital, subject to a "
                f"minimum of {minimum:,.0f} ({pre_money:,.0f} {target_currency})."
            )
    if investment is None:
        investment = float(min(qefr_mins)) if qefr_mins else settings["fallback_investment_pct"] / 100 * pre_money
        assumptions.append(
    ...
    founder_ownership_known = founder_ownership_pct(snapshot) is not None
    if not founder_ownership_known:
        assumptions.append("Founder ownership is unknown; post-round founder percentages cannot be assessed.")
    ...
    def scenario_dict(s):
        ...
        return {
            ...
            "founders_post_round_pct": round(founders_post, 2) if founder_ownership_known else None,
    ...
    return {
        ...
        "snapshot_fingerprint": data_fingerprint(snapshot),
        ...
        "scenario_flags": scenario_flags + ([
            {
                ...
                "severity": "high",
                "detail": (
                    f"Founders fall below {founder_minimum:g}% fully diluted in every "
                    f"modelled scenario ("
                    + ", ".join(
        ...
        if scenarios
        and founder_ownership_known
        and all(
            scenario_dict(s)["founders_post_round_pct"] < founder_minimum
            for s in scenarios
        )
        else []),
        "stamp_duty": {
        ...
        "ownership_by_role_today": {
            k: None if k == "founder" and not founder_ownership_known else round(v, 2)
...
async def captable(
    dataset_name: str, *, pre_money: float | None = None,
    investment: float | None = None, fx_rates: dict[str, float] | None = None,
    currency: str | None = None,
) -> list[InsightFile]:
    """Read selected consolidated data and reuse or generate one Markdown report."""
    insight = InsightFile(dataset_name, "captable", llm_model(), source_datasets=[dataset_name])
    preferred = insight.find(selection="any")
    if preferred is not None and preferred.model == "manual":
        return [preferred]
    source = select_consolidated(dataset_name)
    snapshot = read_build_insight(source)
    ...
    computed["rubric_scope_note"] = (
        "Rubric findings describe the company as of the source date, "
        "not the hypothetical post-round state; see "
        "scenarios[].founders_post_round_pct for post-round ownership."
    )
    computed["rubric"] = apply_rubric(snapshot, settings=settings)
    ...
    narrative = await generate_markdown(
        f"{config['narrative_prompt'].strip()}\n\n### COMPUTED JSON\n\n"
        f"```json\n{json.dumps(computed, ensure_ascii=False, indent=2)}\n```"
    )
    insight.save(render_report(snapshot, computed, narrative))
    logger.info("[%s] Stored capitalization report at %s", dataset_name, insight.path)
    return [insight]
