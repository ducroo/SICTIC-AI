...
1. Run the Python utility:
   ```bash
   {{REPO_ROOT}}/venv/bin/python -m skills.config_load
   ```
...
**Prerequisites:**
- All runtime dependencies (including `rich`) are installed in the project venv at `{{REPO_ROOT}}/venv/` by `{{REPO_ROOT}}/install_skills.sh`.
- The Google Drive must be mounted via rclone to the path set in `GDRIVE_MOUNT` (see `.env`).