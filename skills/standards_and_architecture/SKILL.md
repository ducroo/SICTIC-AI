...
layout. The local root is defined by `LOCAL_STORAGE_PATH`; general application
storage is always local. Optional external synchronization must not be called
from skill code. The domain roots are defined in
...
*(Note on Data Sharing: If a Deal Lead needs access to a specific startup's data, do not break this structure. Share or synchronize a view without changing the configured hierarchy used by the AI.)*
