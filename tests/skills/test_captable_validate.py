...
def test_captable_reviewer_rejects_dropped_rows() -> None:
    ...
    # Complete rows and labelled columns let evidence review distinguish
    # issued holdings, dilution and investment before checking completeness.
    header = "| Holder | Common issued | Preferred seed issued | Diluted shares | Investment |"
    quotes = [
    ...
    for row, quote in zip(output["stakeholders"], quotes):
        row["quote"] = quote
    output["totals"]["quote"] = "| Total | 10000000 | 10570088 | 23070088 | |"
    document = [header, "| --- | --- | --- | --- | --- |", *quotes, output["totals"]["quote"], ""]
    document += ["| Name | Nominal | Votes per share |", "| --- | --- | --- |"]
    for row, quote in zip(output["share_classes"], ["| Common | 0.01 | 1 |", "| Preferred Seed | 0.01 | 1 |"]):
        row["quote"] = quote
        document.append(quote)
    output["pools"][0]["quote"] = "| Equity plans grantable | 865149 | 865149 |"
    document += ["", "| Pool | Total | Unallocated |", "| --- | --- | --- |",
                 output["pools"][0]["quote"], "", output["fully_diluted_definition"]["quote"]]
