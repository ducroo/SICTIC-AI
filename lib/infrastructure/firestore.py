...
from lib.infrastructure.vector_store import FIRESTORE_MAX_VECTOR_DIM  # pragma: allowlist secret
...
class FirestoreAdmin:  # pragma: allowlist secret
    ...
    def delete_collection(self, collection_name: str) -> None:
        chunks = (
            ...
            .document("index")
            .collection(_CHUNKS_SUBCOLLECTION)
        )
        while True:
            docs = list(chunks.limit(400).stream())
            if not docs:
                break
...
class FirestoreAdapter:  # pragma: allowlist secret
    ...
    def delete_dataset(self, dataset_slug: str | None = None) -> bool:
        """Delete this dataset's collection. Firestore stores one dataset per collection."""  # pragma: allowlist secret
        if dataset_slug is not None and slugify(dataset_slug) != self.dataset_slug:
            raise ValueError(
                "FirestoreAdapter.delete_dataset only deletes its own dataset."  # pragma: allowlist secret
            )
        if not self.collection_exists():
            return False
        if self.collection_points_count() == 0:
