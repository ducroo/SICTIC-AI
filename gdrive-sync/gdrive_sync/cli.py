...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gdrive-sync", description="Synchronize STORAGE_MIRROR_PATH with STORAGE_PATH Google Drive root.")
    parser.add_argument("operation", choices=["push", "pull", "sync"])
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--conflict-policy", choices=["local-wins", "cloud-wins"], default="local-wins")
    parser.add_argument("--local-root")
    parser.add_argument("--gdrive-root")
